...
export class SocialShareSeoService {
  ...
  public setLocaleAlternate(locales?: string[]): void {
    if (locales && locales.length) {
      for (const locale of locales) {
        this.metaService.updateTag({property: 'og:locale:alternate', content: locale});
      }
    } else {
  ...
  public setStreetAddress(streetAddress: string): void {
    this.metaService.updateTag({name: 'og:street-address', content: streetAddress});
  }
  ...
  public setCountryName(countryName: string): void {
    this.metaService.updateTag({name: 'og:country-name', content: countryName});
  }
